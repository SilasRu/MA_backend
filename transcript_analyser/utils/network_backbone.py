        ...
        for u in G:
            ...
            if k_out > 1:
                ...
                for v in G.successors(u):
                    ...
                    alpha_ij_out = 1 - \
                        (k_out-1) * integrate.quad(lambda x: (1-x)
                                                   ** (k_out-2), 0, p_ij_out)[0]
                    N.add_edge(u, v, weight=w, alpha_out=float(
                        '%.4f' % alpha_ij_out))
            ...
            if k_in > 1:
                ...
                for v in G.predecessors(u):
                    ...
                    alpha_ij_in = 1 - \
                        (k_in-1) * integrate.quad(lambda x: (1-x)
                                                  ** (k_in-2), 0, p_ij_in)[0]
                    N.add_edge(v, u, weight=w, alpha_in=float(
                        '%.4f' % alpha_ij_in))
        return N
...
if __name__ == '__main__':
    ...
    print('alpha = %s' % alpha)
    print('original: nodes = %s, edges = %s' %
          (G.number_of_nodes(), G.number_of_edges()))
    print('backbone: nodes = %s, edges = %s' %
